...
class Entraineur(models.Model):
    adherent = models.OneToOneField(Adherent, on_delete=models.CASCADE, related_name="entraineur", primary_key= True)
    def str(self):
        return self.adherent.Prenom

class Coach(models.Model):
    entraineur = models.OneToOneField(Entraineur, on_delete=models.CASCADE, primary_key= True)
    def str(self):
        return self.entraineur.adherent.Prenom

class Entrainement(models.Model):
    Dates = models.DateTimeField()
    Lien_fichier = models.CharField(max_length=175)
